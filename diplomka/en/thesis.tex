...
Abstract:
The thesis deals with the use of the newest processor architectures, especially graphics processors (GPU), for parallelization of sophisticated computational problems such as cluster analysis. On this problem, I investigate possibilities of speed-up different types of architectures (CPU and GPU) and especially the dependence of different input data and differnet approaches to parallelization. For example, problem is the divergency of input data (the number of object properties). Furthermore, this thesis delas with on the most efficient use of the architecture like memory bandwith, use the maximum number of cores, minimazing dependecies. Problem is not only in difference between CPU and GPU architecture but also in the different versions of a particular architecture.
